...
export function MarketBuyInterface({ marketId, market }: MarketBuyInterfaceProps) {
    ...
    const checkApproval = async () => {
        ...
        try {
            // Token has 6 decimals, so multiply by 10^6
            const amountInUSDC = BigInt(Math.floor(numericAmount * 1e6));
    ...
    const handleSetApproval = async () => {
        setIsApproving(true);
        try {
            // Approve 1 million tokens (1,000,000 * 10^6) to avoid repeated approvals
            const approvalAmount = BigInt(1_000_000) * BigInt(10**6);
    ...
    const handleConfirm = async () => {
        const numericAmount = parseFloat(amount);
        if (!selectedOption || !amount || isNaN(numericAmount) || numericAmount <= 0) {
    ...
    return (
        ...
        >
            <div 
                ref={contentRef}
                className={cn(
                    "w-full transition-all duration-200 ease-in-out px-2",
                        ...
                        {buyingStep === 'allowance' ? (
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs text-gray-500 mb-1">
                                    {`1 ${selectedOption === 'A' ? market.optionA : market.optionB} = 1 token`}
                                </span>
